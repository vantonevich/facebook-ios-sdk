                    ...
                    orError:(NSError *)errorParam
{
    ...
    for (NSUInteger i = 0; i < count; i++) {
        ...
        NSUInteger resultIndex = errorParam == itemError ? i : 0;
        ...
        if ((metadata.request.session.accessTokenData.loginType == FBSessionLoginTypeSystemAccount) &&
            ...
            if (metadata.request.session.accessTokenData.loginType == FBSessionLoginTypeSystemAccount){
                ...
                if ([self isExpiredTokenError:itemError resultIndex:resultIndex]
                    ...
                    taskWork = [taskWork completionTaskWithQueue:dispatch_get_main_queue() block:^id(FBTask *taskOuter) {
                        if (taskOuter.result == ACAccountCredentialRenewResultRenewed) {
...
+ (void)addRequestToRefreshPermissionsSession:(FBSession*)session connection:(FBRequestConnection*)connection {
    ...
    [connection addRequest:request
         completionHandler:^(FBRequestConnection *connectionInner, id result, NSError *error) {
